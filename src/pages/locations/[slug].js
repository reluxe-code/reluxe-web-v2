          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-end">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <div className="flex items-center gap-2 mb-3">
                <a href="/locations" className="inline-flex items-center gap-1.5 transition-colors duration-200" style={{ fontFamily: fonts.body, fontSize: '0.8125rem', fontWeight: 500, color: 'rgba(250,248,245,0.5)', textDecoration: 'none' }}>
          ...
          <div className="max-w-7xl mx-auto px-6 py-16 lg:py-20">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {staff.map((s, i) => {
                ...
                return (
                  <motion.a key={s.slug} href={`/team/${s.slug}`} className="group rounded-2xl overflow-hidden" style={{ backgroundColor: '#fff', border: `1px solid ${colors.stone}`, textDecoration: 'none' }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.04 }} whileHover={{ y: -4 }}>
        ...
        <div className="max-w-7xl mx-auto px-6 -mt-8 mb-8">
          <motion.div className="rounded-xl py-3 px-4" style={{ borderLeft: `2px solid ${colors.violet}`, background: `${colors.violet}06` }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            <p style={{ fontFamily: fonts.body, fontSize: '0.8125rem', color: colors.body, lineHeight: 1.5 }}>
              ...
              <a href="/locations/westfield" style={{ color: colors.violet, fontWeight: 600, textDecoration: 'underline' }}>View Westfield services</a>.
            </p>
          </motion.div>
        </div>
      )}
        ...
        <div className="max-w-4xl mx-auto px-6 py-16 lg:py-20 text-center relative">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <a href={`/locations/${otherSlug}`} className="rounded-full transition-all duration-200 hover:shadow-lg" style={{ fontFamily: fonts.body, fontSize: '0.9375rem', fontWeight: 600, padding: '0.875rem 2.25rem', background: gradients.primary, color: '#fff', textDecoration: 'none', display: 'inline-block' }}>
                Visit {otherCity}
              </a>
              <GravityBookButton fontKey={fontKey} size="hero" />
            </div>
...
export default function BetaLocationDetail({ location, staff, services, testimonials, globalBundles }) {
  const city = location?.locationFields?.city || 'Location';
  const slug = location?.slug || '';
  const canonical = `https://reluxemedspa.com/locations/${slug}`;
  const fullAddress = location?.locationFields?.fullAddress || '';
  const phone = location?.locationFields?.phone || '(317) 763-1142';
  const lat = location?.locationFields?.locationMap?.latitude;
  const lng = location?.locationFields?.locationMap?.longitude;
  const locFaqs = location?.locationFields?.faqs || [];
    ...
    '@graph': [
      {
        ...
        '@id': `${canonical}#loc`,
        name: `RELUXE Med Spa — ${city}`,
        url: canonical,
        telephone: phone,
        priceRange: '$$',
        parentOrganization: { '@id': 'https://reluxemedspa.com#org' },
        ...(fullAddress ? { address: { '@type': 'PostalAddress', streetAddress: fullAddress.split(',')[0], addressLocality: city, addressRegion: 'IN', addressCountry: 'US' } } : {}),
        ...(lat && lng ? { geo: { '@type': 'GeoCoordinates', latitude: lat, longitude: lng } } : {}),
        hasOfferCatalog: {
          '@type': 'OfferCatalog',
          name: `Services at RELUXE ${city}`,
          itemListElement: services.slice(0, 15).map(s => ({
            '@type': 'Offer',
            itemOffered: { '@type': 'Service', name: s.name, url: `https://reluxemedspa.com/services/${s.slug}` },
          })),
        },
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Locations', item: 'https://reluxemedspa.com/locations' },
  ...
  return (
    <BetaLayout
      title={`RELUXE Med Spa ${city}, IN — Botox, Facials, Morpheus8 & More`}
      description={`Visit RELUXE Med Spa in ${city}, Indiana. Expert Botox, Daxxify, facials, Morpheus8, fillers & laser treatments. ${fullAddress ? fullAddress + '. ' : ''}Book your free consultation today.`}
      canonical={canonical}
