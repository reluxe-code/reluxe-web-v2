...
export default function BetaAbout({ staff }) {
  return (
    <BetaLayout title="About RELUXE Med Spa — Luxury Aesthetics in Westfield & Carmel, IN" description="RELUXE Med Spa in Carmel and Westfield is redefining luxury aesthetics in Indiana. Expert Botox injectors, advanced technology, and natural results. Family-owned, 5-star rated." canonical="https://reluxemedspa.com/about">
      {({ fontKey, fonts }) => (
        <>
          {/* Hero */}
              ...
              <div className="max-w-7xl mx-auto px-6 py-24 lg:py-32">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
                  {staff.slice(0, 8).map((member, i) => (
                    <motion.a key={member.slug} href={`/team/${member.slug}`} className="group block rounded-2xl overflow-hidden" style={{ textDecoration: 'none', backgroundColor: colors.cream, border: `1px solid ${colors.stone}` }} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.06 }}>
