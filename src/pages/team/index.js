            ...
            <motion.div key="r" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-3xl mx-auto mb-8">
                {matchedProviders.map((p) => {
                  ...
                  return (
                    <a key={p.slug} href={`/team/${p.slug}`} className="rounded-xl p-5 text-center transition-all duration-200 hover:border-violet-500/30" style={{ backgroundColor: 'rgba(250,248,245,0.04)', border: '1px solid rgba(250,248,245,0.1)', textDecoration: 'none' }}>
...
export default function BetaTeamIndex({ staffList, testimonials }) {
  const structuredData = {
    ...
    '@graph': [
      {
        '@type': 'ItemList',
        name: 'RELUXE Med Spa Expert Providers',
        description: 'Meet the expert injectors, aestheticians, and wellness professionals at RELUXE Med Spa in Westfield & Carmel, Indiana.',
        numberOfItems: staffList.length,
        itemListElement: staffList.map((s, i) => ({
          '@type': 'ListItem',
          position: i + 1,
          item: {
            '@type': 'Person',
            name: s.name || s.title,
            jobTitle: s?.staffFields?.staffTitle || s?.staffFields?.role || '',
  ...
  return (
    <BetaLayout
      title="Expert Med Spa Providers — Injectors & Aestheticians"
      description="Meet the expert Botox injectors, aestheticians & wellness providers at RELUXE Med Spa. NPs, PAs & licensed professionals in Westfield & Carmel, IN. Book your provider today."
      canonical="https://reluxemedspa.com/team"
