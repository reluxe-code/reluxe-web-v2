...
export default function BetaResults() {
  ...
  async function handleUnlock(e) {
    ...
    try {
      await fetch('/api/leads/capture', {
        ...
        body: JSON.stringify({ email, source: 'results-gallery', tags: ['results-unlock'] }),
      });
    } catch {
  ...
  return (
    <BetaLayout title="Before & After Results — Botox, Fillers, Morpheus8 & More" description="See real before & after results from Botox, dermal fillers, Morpheus8, SkinPen, and laser treatments at RELUXE Med Spa in Westfield & Carmel, IN." canonical="https://reluxemedspa.com/results">
      {({ fontKey, fonts }) => (
        <>
          {/* Hero */}
