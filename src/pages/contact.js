...
const LOCATION_GRADIENTS = {
  westfield: 'linear-gradient(135deg, #7C3AED, #5B21B6)',
  carmel: 'linear-gradient(135deg, #C026D3, #9333EA)',
};
...
const locations = LOCATIONS.map((loc) => ({
  ...
  mapUrl: loc.mapUrl,
  gradient: LOCATION_GRADIENTS[loc.key] || 'linear-gradient(135deg, #7C3AED, #5B21B6)',
}));

const contactMethods = [
  { label: 'Call Us', value: LOCATIONS[0]?.phone || '(317) 763-1142', href: `tel:${(LOCATIONS[0]?.phone || '').replace(/\D/g, '')}`, icon: 'phone' },
  { label: 'Email Us', value: 'help@reluxemedspa.com', href: 'mailto:help@reluxemedspa.com', icon: 'mail' },
  { label: 'Text Us', value: LOCATIONS[0]?.phone || '(317) 763-1142', href: `sms:${(LOCATIONS[0]?.phone || '').replace(/\D/g, '')}`, icon: 'message' },
];
...
export default function BetaContact() {
    ...
    try {
      await fetch('/api/leads/capture', {
        ...
        body: JSON.stringify({
          ...
          source: 'contact-form',
          tags: ['contact-form', form.location].filter(Boolean),
        }),
      });
      setStatus('sent');
  ...
  return (
    <BetaLayout title="Contact RELUXE Med Spa — Westfield & Carmel, IN" description="Get in touch with RELUXE Med Spa in Carmel and Westfield, Indiana. Call (317) 763-1142, send us a message, or book your free Botox & facial consultation online." canonical="https://reluxemedspa.com/contact">
      {({ fontKey, fonts }) => (
        <>
          {/* Hero */}
