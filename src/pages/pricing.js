...
// Fallback mapping — only used when dynamic slug lookup fails
const NAME_TO_SLUG = [
...
// Build slug lookup from dynamic services list (populated via getStaticProps)
let _serviceSlugMap = null;
function buildSlugMap(services) {
  if (_serviceSlugMap) return _serviceSlugMap;
  _serviceSlugMap = new Map();
  for (const svc of services || []) {
    // Map slug itself, name, and common variations
    if (svc.slug) _serviceSlugMap.set(svc.slug.toLowerCase(), svc.slug);
    if (svc.name) _serviceSlugMap.set(svc.name.toLowerCase(), svc.slug);
    if (svc.title) _serviceSlugMap.set(svc.title.toLowerCase(), svc.slug);
  }
  return _serviceSlugMap;
}

function guessSlug(label, services) {
  const s = (label || '').toLowerCase().trim();
  if (!s) return '';

  // Try dynamic map first
  const slugMap = buildSlugMap(services);
  if (slugMap.has(s)) return slugMap.get(s);
  // Try partial match against dynamic slugs
  for (const [key, slug] of slugMap) {
    if (s.includes(key) || key.includes(s)) return slug;
  }
...
function CategoryCard({ group, fonts, index, services }) {
  return (
    <motion.div
      ...
      <div className="p-5 lg:p-6">
        ...
        <div className="space-y-0">
          {group.items.map((item, i) => {
            const slug = guessSlug(item.name, services);
            const inner = (
            ...
            return slug ? (
              <a key={item.name} href={`/services/${slug}`} className="block hover:bg-stone/30 rounded-lg px-2 -mx-2 transition-colors" style={{ textDecoration: 'none' }}>{inner}</a>
            ) : (
...
export default function BetaPricing({ services = [] }) {
  ...
  return (
    <BetaLayout title="Med Spa Pricing — Botox, Fillers, Facials & Laser Treatments" description="Transparent pricing for Botox ($10/unit), Daxxify, dermal fillers, Morpheus8, facials, laser hair removal & more at RELUXE Med Spa in Westfield & Carmel, IN. No hidden fees." canonical="https://reluxemedspa.com/pricing">
      {({ fontKey, fonts }) => (
        <>
          {/* Hero */}
            ...
            <div className="max-w-7xl mx-auto px-6 py-12 lg:py-16">
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredGroups.map((group, i) => (
                  <CategoryCard key={group.title} group={group} fonts={fonts} index={i} services={services} />
                ))}
              </div>
            </div>
          </section>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <motion.div className="rounded-2xl overflow-hidden relative" style={{ background: gradients.primary, minHeight: 300 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
                  ...
                  <div className="relative z-10 p-8 lg:p-10 flex flex-col justify-between h-full" style={{ minHeight: 300 }}>
                    ...
                    <a href="/memberships" className="rounded-full self-start mt-6" style={{ fontFamily: fonts.body, fontSize: '0.9375rem', fontWeight: 600, padding: '0.875rem 2.25rem', backgroundColor: '#fff', color: colors.ink, textDecoration: 'none', display: 'inline-block' }}>Learn About Membership</a>
                  </div>
                </motion.div>
...
export async function getStaticProps() {
  ...
  try {
    ...
    services = allServices.map((s) => ({ slug: s.slug, name: s.name, title: s.name }));
  } catch (e) {
    console.warn('Beta pricing: could not load services list', e.message);
  }
