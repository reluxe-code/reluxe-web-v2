...
export default function BetaInspiration({ articles }) {
  ...
  return (
    <BetaLayout title="Skincare Tips & Treatment Guides" description="Skincare tips, Botox guides, facial treatment ideas & expert advice from the RELUXE Med Spa team in Westfield & Carmel, IN." canonical="https://reluxemedspa.com/inspiration">
      {({ fontKey, fonts }) => (
        <>
          {/* Hero */}
...
function NewsletterSection({ fonts }) {
  ...
  async function handleSubscribe(e) {
    ...
    try {
      await fetch('/api/leads/capture', {
        ...
        body: JSON.stringify({ email: nlEmail, source: 'newsletter', tags: ['newsletter'] }),
      });
      setNlStatus('done');
    } catch {
      setNlStatus('error');
    ...
    <section style={{ backgroundColor: colors.ink, position: 'relative' }}>
      ...
      <div className="max-w-3xl mx-auto px-6 py-20 lg:py-28 text-center relative">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          {nlStatus === 'done' ? (
            <p style={{ fontFamily: fonts.body, fontSize: '1rem', fontWeight: 600, color: colors.violet }}>You&apos;re subscribed!</p>
          ) : (
            <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <input type="email" value={nlEmail} onChange={(e) => setNlEmail(e.target.value)} placeholder="your@email.com" required className="flex-1 rounded-full px-5 py-3.5 outline-none" style={{ fontFamily: fonts.body, fontSize: '0.9375rem', backgroundColor: 'rgba(250,248,245,0.06)', border: '1px solid rgba(250,248,245,0.08)', color: colors.white }} />
              <button type="submit" disabled={nlStatus === 'sending'} className="rounded-full flex-shrink-0" style={{ fontFamily: fonts.body, fontSize: '0.9375rem', fontWeight: 600, padding: '0.875rem 2rem', background: gradients.primary, color: '#fff', border: 'none', cursor: 'pointer', opacity: nlStatus === 'sending' ? 0.6 : 1 }}>
                {nlStatus === 'sending' ? 'Subscribing...' : 'Subscribe'}
              </button>
            </form>
          )}
          {nlStatus === 'error' && <p style={{ fontFamily: fonts.body, fontSize: '0.8125rem', color: '#ef4444', marginTop: '0.5rem' }}>Something went wrong. Please try again.</p>}
