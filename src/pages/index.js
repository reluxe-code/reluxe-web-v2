...
const featuredServices = [
  { name: 'Tox', tagline: 'Botox, Jeuveau, Dysport & Daxxify.', href: '/services/tox', badge: 'Most Popular', size: 'large', gradient: 'linear-gradient(135deg, #7C3AED, #5B21B6)' },
  { name: 'Dermal Fillers', tagline: 'Cheeks. Lips. Jawline. Yes, all of it.', href: '/services/filler', badge: 'Instant Results', size: 'standard', gradient: 'linear-gradient(135deg, #C026D3, #9333EA)' },
  { name: 'Morpheus8', tagline: 'Skin tightening that actually works.', href: '/services/morpheus8', badge: '#1 Skin Remodel', size: 'standard', gradient: 'linear-gradient(135deg, #E11D73, #C026D3)' },
  { name: 'Laser Hair Removal', tagline: 'Razor? Don\u2019t know her.', href: '/services/laser-hair-removal', badge: 'Lose the Razor', size: 'standard', gradient: 'linear-gradient(135deg, #5B21B6, #1E1B4B)' },
  { name: 'Lasers & Skin', tagline: 'IPL, ClearLift, CO\u2082 \u2014 pick your weapon.', href: '/services/lasers', badge: 'Advanced Tech', size: 'standard', gradient: 'linear-gradient(135deg, #9333EA, #7C3AED)' },
  { name: 'Facials & Peels', tagline: 'Because great skin isn\u2019t luck. It\u2019s us.', href: '/services/facials', badge: 'Fan Favorite', size: 'large', gradient: 'linear-gradient(135deg, #1A1A1A, #2A2A2A)' },
];
...
const memberPerks = ['One service every month (or save for tox)', '10% off other services', '10% off packages', '15% off products', 'Best pricing on tox', 'Additional VIP benefits'];
...
// Hours by day-of-week index (0=Sun, 1=Mon, ..., 6=Sat)
const LOCATION_HOURS = {
  westfield: [null, '9am\u20135pm', '9am\u20135pm', '9am\u20135pm', '9am\u20137pm', '9am\u20135pm', '9am\u20133pm'],
  carmel:    [null, '9am\u20135pm', '9am\u20135pm', '9am\u20135pm', '9am\u20135pm', '9am\u20135pm', null],
};

function OpenTodayBanner({ fonts }) {
  const day = new Date().getDay();
  const westfieldHours = LOCATION_HOURS.westfield[day];
  const carmelHours = LOCATION_HOURS.carmel[day];
  const anyOpen = westfieldHours || carmelHours;
      ...
      <div className="max-w-6xl mx-auto px-6 py-4">
        <div className="flex flex-wrap items-center justify-center gap-6">
          <div className="flex items-center gap-3">
            <span style={{ fontFamily: fonts.body, fontSize: '0.75rem', fontWeight: 600, color: 'rgba(250,248,245,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              {anyOpen ? 'Open Today' : 'Closed Today'}
            </span>
            {westfieldHours && (
              <div className="flex items-center gap-2">
                <span style={{ fontFamily: fonts.body, fontSize: '0.8125rem', fontWeight: 500, color: 'rgba(250,248,245,0.6)' }}>Westfield</span>
                <span style={{ fontFamily: fonts.body, fontSize: '0.6875rem', fontWeight: 500, color: colors.violet }}>{westfieldHours}</span>
                <ScarcityBadge locationKey="westfield" variant="inline" fonts={fonts} />
              </div>
            )}
            {westfieldHours && carmelHours && <span style={{ color: 'rgba(250,248,245,0.15)' }}>|</span>}
            {carmelHours && (
              <div className="flex items-center gap-2">
                <span style={{ fontFamily: fonts.body, fontSize: '0.8125rem', fontWeight: 500, color: 'rgba(250,248,245,0.6)' }}>Carmel</span>
                <span style={{ fontFamily: fonts.body, fontSize: '0.6875rem', fontWeight: 500, color: colors.violet }}>{carmelHours}</span>
                <ScarcityBadge locationKey="carmel" variant="inline" fonts={fonts} />
              </div>
            )}
            {!anyOpen && (
              <span style={{ fontFamily: fonts.body, fontSize: '0.8125rem', color: 'rgba(250,248,245,0.4)' }}>We&rsquo;re back Monday at 9am</span>
            )}
          </div>
        </div>
      </div>
  ...
  '@graph': [
    {
      ...
      description: 'Premier med spa in Westfield & Carmel, Indiana offering Botox, Daxxify, Morpheus8, facials, dermal fillers, laser hair removal, and advanced skin treatments.',
      sameAs: [
      ...
      contactPoint: [{
        ...
        areaServed: ['Westfield IN', 'Carmel IN', 'Fishers IN', 'Noblesville IN', 'Zionsville IN', 'Indianapolis IN'],
    ...
    },
    {
      ...
      address: { '@type': 'PostalAddress', streetAddress: '514 E State Road 32', addressLocality: 'Westfield', addressRegion: 'IN', postalCode: '46074', addressCountry: 'US' },
    ...
    {
      '@type': 'WebSite',
      '@id': 'https://reluxemedspa.com#website',
      name: 'RELUXE Med Spa',
      url: 'https://reluxemedspa.com',
      publisher: { '@id': 'https://reluxemedspa.com#org' },
    },
    {
      '@type': 'FAQPage',
...
export default function BetaHome({ testimonials, staff, featuredStories = [] }) {
  ...
  return (
    <BetaLayout
      rawTitle="RELUXE Med Spa | Botox, Daxxify, Morpheus8, Facials & Injections | Westfield & Carmel, IN"
      title="Botox, Daxxify, Morpheus8, Facials & Injections in Westfield & Carmel, IN"
      description="Indiana's premier med spa. Expert Botox, Daxxify, Morpheus8, facials, dermal fillers & laser treatments in Westfield & Carmel, IN. 5-star rated, 300+ reviews. Book your free consultation today."
      canonical="https://reluxemedspa.com"
          ...
          <section style={{ backgroundColor: '#fff' }}>
            <div className="max-w-7xl mx-auto px-6 py-24 lg:py-32">
              ...
              <motion.div className="text-center mt-12" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 }}>
                <a href="/services" className="inline-flex items-center justify-center rounded-full" style={{ background: gradients.primary, color: '#fff', padding: '0.875rem 2.5rem', fontSize: '1rem', fontFamily: fonts.body, fontWeight: 600, textDecoration: 'none', border: 'none' }}>View All Treatments</a>
              </motion.div>
            </div>
          </section>
            ...
            <div className="max-w-7xl mx-auto px-6 py-24 lg:py-36 relative">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
                ...
                <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
                  ...
                  <a href="/about" className="inline-flex items-center gap-2 group" style={{ fontFamily: fonts.body, fontSize: '0.9375rem', fontWeight: 600, color: colors.violet, textDecoration: 'none' }}>
          ...
          <section ref={teamRef} style={{ backgroundColor: colors.cream, position: 'relative', overflow: 'hidden' }}>
            <div className="max-w-7xl mx-auto px-6 pt-24 lg:pt-32 pb-8">
              ...
              <motion.div className="text-center mt-8 pb-8" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 }}>
                <a href="/team" className="rounded-full inline-flex items-center" style={{ fontFamily: fonts.body, fontSize: '0.875rem', fontWeight: 600, color: colors.violet, border: `1.5px solid ${colors.violet}`, padding: '0.625rem 1.75rem', textDecoration: 'none' }}>Meet All Providers &rarr;</a>
              </motion.div>
            </div>
          </section>
                ...
                <motion.div className="rounded-2xl overflow-hidden relative" style={{ background: gradients.primary, minHeight: 340 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
                  ...
                  <div className="relative z-10 p-8 lg:p-10 flex flex-col justify-between h-full" style={{ minHeight: 340 }}>
                    <div>
                      ...
                      <h3 style={{ fontFamily: fonts.display, fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', fontWeight: 700, color: '#fff', marginBottom: '0.75rem' }}>VIP Membership</h3>
                      <p style={{ fontFamily: fonts.body, fontSize: '1rem', color: 'rgba(255,255,255,0.75)', lineHeight: 1.6, maxWidth: '22rem', marginBottom: '1.5rem' }}>Monthly treatments, exclusive perks, and member-only pricing starting at $99/month.</p>
                    ...
                    <a href="/memberships" className="rounded-full self-start" style={{ fontFamily: fonts.body, fontSize: '0.9375rem', fontWeight: 600, padding: '0.875rem 2.25rem', backgroundColor: '#fff', color: colors.ink, border: 'none', cursor: 'pointer', textDecoration: 'none', display: 'inline-block' }}>Learn About Membership</a>
                  </div>
                </motion.div>
          ...
          <section style={{ backgroundColor: '#fff' }}>
            <div className="max-w-4xl mx-auto px-6 py-24 lg:py-32">
              ...
              <motion.div className="text-center mt-12" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }}>
                ...
                <a href="/contact" className="rounded-full inline-flex items-center" style={{ fontFamily: fonts.body, fontSize: '0.875rem', fontWeight: 600, color: colors.violet, border: `1.5px solid ${colors.violet}`, padding: '0.625rem 1.75rem', textDecoration: 'none' }}>Contact Us &rarr;</a>
              </motion.div>
            </div>
          </section>

          {/* ─── Colorescience Anniversary Sale ─── */}
          <section style={{ background: 'linear-gradient(135deg, #FDF6F0 0%, #FFF5EE 50%, #FDF6F0 100%)', position: 'relative' }}>
            <div className="max-w-5xl mx-auto px-6 py-20 lg:py-28">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center">
                <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 mb-6" style={{ background: 'rgba(207,113,85,0.1)', border: '1px solid rgba(207,113,85,0.25)' }}>
                  <span style={{ fontFamily: fonts.body, fontSize: '0.6875rem', fontWeight: 600, color: '#CF7155', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Limited Time</span>
                </div>
                <h2 style={{ fontFamily: fonts.display, fontSize: 'clamp(1.75rem, 4vw, 2.75rem)', fontWeight: 700, color: '#CF7155', marginBottom: '0.5rem', fontStyle: 'italic' }}>Colorescience Anniversary Sale</h2>
                <p style={{ fontFamily: fonts.body, fontSize: '1.0625rem', lineHeight: 1.6, color: colors.body, maxWidth: '32rem', margin: '0 auto 2rem' }}>20% off sitewide &mdash; no exclusions. Plus free shipping on every order through our partnership link.</p>

                <div className="max-w-lg mx-auto rounded-2xl p-8" style={{ backgroundColor: '#fff', border: '1px solid rgba(207,113,85,0.15)', boxShadow: '0 4px 24px rgba(207,113,85,0.08)' }}>
                  <div className="space-y-3 text-left mb-6">
                    {[
                      { text: 'March 5th – 22nd', bold: true },
                      { text: '20% discount sitewide, no product exclusions' },
                      { text: 'FREE shipping on all orders' },
                      { text: 'FREE Travel Size Barrier Pro Serum Spray with orders $125+' },
                      { text: 'FREE Total Protection\u00AE Sport Stick SPF 50 with orders $250+' },
                    ].map((item, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <span style={{ color: '#CF7155', fontWeight: 700, fontSize: '1rem', lineHeight: '1.5rem', flexShrink: 0 }}>+</span>
                        <span style={{ fontFamily: fonts.body, fontSize: '0.9375rem', color: colors.heading, fontWeight: item.bold ? 700 : 400 }}>{item.text}</span>
                      </div>
                    ))}
                  </div>
                  <a href="https://colorescience.com/reluxe-med-spa" target="_blank" rel="noopener noreferrer" className="block w-full rounded-xl py-3.5 text-center" style={{ fontFamily: fonts.body, fontSize: '1rem', fontWeight: 600, background: 'linear-gradient(135deg, #CF7155, #B85A40)', color: '#fff', border: 'none', cursor: 'pointer', textDecoration: 'none' }}>Shop the Sale &rarr;</a>
                  <p className="mt-3 text-center" style={{ fontFamily: fonts.body, fontSize: '0.75rem', color: colors.muted }}>You&rsquo;ll be redirected to Colorescience &mdash; RELUXE is an authorized retail partner.</p>
                </div>
              </motion.div>
            </div>
          </section>
