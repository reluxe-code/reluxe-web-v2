...
import MadlibsExplorer from '@/components/experiments/MadlibsExplorer';
...
export default function MadlibsExperiment({ testimonials, staff }) {
  ...
  return (
    <BetaLayout
      title="Curate Your Perfect Visit"
      description="Build your dream appointment in seconds. Choose your treatment, pick your provider, and find available slots instantly."
      noindex
    >
      {({ fontKey, fonts }) => (
        <>
          ...
          {/* ─── Mad-Libs Appointment Explorer ─── */}
          <MadlibsExplorer fonts={fonts} fontKey={fontKey} staff={staff} />
...
export async function getStaticProps() {
  ...
  try {
    ...
    const [testimonialsRes, staffRes] = await Promise.all([
      ...
      sb
        .from('staff')
        .select('id, slug, name, title, role, featured_image, transparent_bg, fun_fact, locations, specialties, boulevard_provider_id, boulevard_service_map')
        .eq('status', 'published')
        .order('sort_order')
        .order('name')
        .limit(12),
...
MadlibsExperiment.getLayout = (page) => page;
