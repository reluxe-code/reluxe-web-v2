...
const OVERRIDES = {
  ...
  lashlift: {
    westfield: {
      ...
      durationText: 'Typically 45–60 minutes',
      description:
        'Lift and tint enhances your natural lashes for a wide-awake look without extensions. Results last 6–8 weeks. Sessions take 45–60 minutes.',
      differences: [
        'Gentle formulations to protect lash health',
        'Curl angle chosen to suit eye shape',
      ],
      faqs: [
        fq('Wet lashes after?', 'Keep dry for 24 hours to set the curl.'),
        fq('Mascara needed?', 'Usually not; tint gives definition.'),
        fq('Safe for sensitive eyes?', 'Yes—patch available if needed.'),
        fq('How long it lasts?', 'Around 6–8 weeks depending on growth.'),
        fq('Can I use a serum?', 'Yes—conditioners help longevity.'),
        fq('Remove makeup before?', 'Arrive with clean lashes for best results.'),
      ],
    },
    carmel: {
      exists: true,
      durationText: 'Typically 45–60 minutes',
      description:
        'A low-maintenance lash lift with optional tint for natural, eye-opening curl. Expect ~6–8 weeks of wear. Appointments run 45–60 minutes.',
      differences: [
        'Soft curl options for hooded lids',
        'Tint tones tailored to your coloring',
      ],
      faqs: [
        fq('Oil-based removers?', 'Avoid for the first day; gentle removers after.'),
        fq('Swim safe?', 'After 24 hours you’re good to go.'),
        fq('Mascara?', 'You can—just skip waterproof to reduce dryness.'),
        fq('Allergy risk?', 'Low; let us know if you’ve reacted to tints before.'),
        fq('Repeat timing?', 'Every 6–8 weeks aligns with lash growth.'),
        fq('Aftercare?', 'Keep dry 24 hours; brush daily to maintain shape.'),
      ],
    },
  },
  ...
  browwax: {
    westfield: {
      exists: true,
      durationText: 'Typically 20 minutes',
      description:
        'Brow shaping to refine arches and open the eye area with clean, natural lines. Session time is about 20 minutes.',
      differences: [
        'Face mapping for brow balance',
        'Soothing finish to reduce redness',
      ],
      faqs: [
        fq('Tint available?', 'Yes—ask to add brow tint for definition.'),
        fq('Sensitive skin?', 'We use gentle wax and soothing post-care.'),
        fq('Redness time?', 'Usually fades within 30–60 minutes.'),
        fq('Regrowth timing?', 'Most return every 4–6 weeks.'),
        fq('Breakout risk?', 'Low; we prep the skin and avoid double dipping.'),
        fq('Makeup after?', 'Light application is fine once redness calms.'),
      ],
    },
    carmel: {
      exists: true,
      durationText: 'Typically 20 minutes',
      description:
        'Clean, soft shaping that flatters your natural brow growth without harsh lines. Appointments take ~20 minutes.',
      differences: [
        'Measured symmetry with flexible styling',
        'Cooling compress post-wax for comfort',
      ],
      faqs: [
        fq('Grow out first?', 'A little growth helps us refine your shape.'),
        fq('Threading vs wax?', 'We default to gentle wax; threading by request.'),
        fq('Aftercare?', 'Avoid retinoids and exfoliants around the brows for 48 hours.'),
        fq('Patch test?', 'Available for very sensitive skin.'),
        fq('How often?', 'Every 4–6 weeks maintains shape.'),
        fq('Tint too?', 'Yes—great for fullness without makeup.'),
      ],
    },
  },
