...
const TAG_ID = 'urn:blvd:Tag:87cd9be2-12c6-476b-ae9b-ce4624d5ee04'

// Get the same 131 clients
const { data: dueClients, error } = await db
  .from('client_tox_summary')
  .select('client_id, boulevard_id')
  .eq('tox_segment', 'due')

if (error) { console.error(error.message); process.exit(1) }

const clients = dueClients.filter(c => c.boulevard_id)
console.log(`Removing "${TAG_NAME}" from ${clients.length} clients...`)
...
for (const c of clients) {
  try {
    ...
    `, { input: { entityId: c.boulevard_id, tagId: TAG_ID } })
    removed++
    if (removed % 25 === 0) console.log(`  ...removed ${removed}/${clients.length}`)
  } catch (err) {
    failed++
    if (failed <= 3) console.error(`  Failed ${c.boulevard_id}: ${err.message.slice(0, 100)}`)
  }
}
